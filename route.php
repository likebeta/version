...
if (isset($route_params['action'])) {
	$controler = $route_params['model'] .'-'.$route_params['action'];
}
else if (isset($route_params['model'])) {
...
function formatRouteUrl()
{
	...
	if ($count > 1) {
		$route_params['action'] = $tmp_array[1];
	}

	if ($count > 2) {
		$route_params['params'] = array_slice($tmp_array,2);
	}
